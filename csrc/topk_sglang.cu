/**
 * Vortex TopK kernel — mirrors topk_slgang_ori.cu structure with additions:
 * - bf16 support, flexible radix, mapping/remap modes
 * - CSR paged wrapper kernels for vortex integration
 * Profiling kernels are in topk_sglang_profile.cu.
 */
    ...
    const int64_t     radix_bits)
{
    ...
    TORCH_CHECK(radix_bits >= 4 && radix_bits <= 10,
                "topk_output: radix_bits must be 4-10, got ", radix_bits);
