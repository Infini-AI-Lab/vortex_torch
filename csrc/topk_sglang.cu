...
constexpr int VORTEX_MAX_TOPK = 4096;
...
// Adapted from topk_slgang_ori.cu — uses direct convert_to_uint8()
// for Stage 1 binning with no pre-pass, no LUT, no bin cache.
// ======================================================================
    ...
    int           target_k)
{
    ...
    // Stage 1: 8-bit coarse histogram (direct convert_to_uint8, no mapping)
    if (tx < RADIX + 1) s_histogram[tx] = 0;
    __syncthreads();
    ...
    int*          counters = nullptr)
{
    ...
    if (needs_auto_range(mapping.mode) && !mapping.noscale) {
        ...
        // Lightweight topk-window pre-pass: compute min/max of raw values,
        // then focus all 256 bins on [tau_low, max] where
        // tau_low = max - (max - min) * rho * k / length.
        // Like mode 12 but uses a simple heuristic instead of quantile estimation.
        float local_min = __FLT_MAX__, local_max = -__FLT_MAX__;
        for (int idx = tx; idx < length; idx += BLOCK_SIZE) {
            float val = vortex_to_float(input[idx + row_start]);
            local_min = fminf(local_min, val);
        ...
        __shared__ float s_warp_mins_tw2[32], s_warp_maxs_tw2[32];
        {
            int warp_id = tx >> 5, lane_id = tx & 31;
            if (lane_id == 0) { s_warp_mins_tw2[warp_id] = local_min; s_warp_maxs_tw2[warp_id] = local_max; }
        }
        __syncthreads();
        if (tx < (BLOCK_SIZE >> 5)) {
            local_min = s_warp_mins_tw2[tx]; local_max = s_warp_maxs_tw2[tx];
            for (int offset = 16; offset > 0; offset >>= 1) {
                local_min = fminf(local_min, __shfl_xor_sync(0xFFFFFFFF, local_min, offset));
                local_max = fmaxf(local_max, __shfl_xor_sync(0xFFFFFFFF, local_max, offset));
            }
            if (tx == 0) {
                ...
                if (rho <= 0.0f) rho = 4.0f;
                int k = (mapping.target_k > 0) ? mapping.target_k : target_k;
                float frac = rho * float(k) / float(length);
                frac = fminf(frac, 1.0f);
                float tau_low = local_max - (local_max - local_min) * frac;
                if (tau_low >= local_max) tau_low = local_min;
                float range = local_max - tau_low;
                s_range_min = tau_low;
                s_range_inv_range = (range > 1e-10f) ? 255.0f / range : 0.0f;
            }
...
// Ori fast-path wrapper: zero mapping overhead
template <typename ScoreT>
__global__ __launch_bounds__(kThreadsPerBlock)
    ...
    const int     page_reserved_eos)
{
    ...
    __shared__ int s_indices[VORTEX_MAX_TOPK];
    fast_topk_ori<ScoreT>(score_blk, s_indices, 0, nblk, topk_val);
    __syncthreads();
    ...
    const bool        mapping_noscale)
{
    ...
    // Fast path for mode 0 (MAPPING_NONE): use ori kernel with zero mapping overhead
    if (mapping_mode == MAPPING_NONE) {
        if (x.scalar_type() == at::ScalarType::BFloat16) {
            setup_kernel_smem_once<TopKOutput_Ori_Kernel<__nv_bfloat16>, kSmem>();
            TopKOutput_Ori_Kernel<__nv_bfloat16><<<nblks, nthreads, kSmem, stream>>>(
            ...
            setup_kernel_smem_once<TopKOutput_Ori_Kernel<float>, kSmem>();
            TopKOutput_Ori_Kernel<float><<<nblks, nthreads, kSmem, stream>>>(
                x.data_ptr<float>(),
                dense_kv_indptr.data_ptr<int>(),
                sparse_kv_indptr.data_ptr<int>(),
                dense_kv_indices.data_ptr<int>(),
