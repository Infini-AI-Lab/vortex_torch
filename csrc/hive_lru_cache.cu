...
// =============================================================================
// Hive-Style Lock-Free LRU Cache Allocation (Optimized Design)
// =============================================================================
//
// Key design principles:
// 1. LOCK-FREE FAST PATH for cache hits (no seqlock, just atomics)
// 2. SEQLOCK SLOW PATH for allocations/evictions (correctness guarantee)
// 3. Timestamp-based LRU using monotonic counters (concurrency-friendly)
// 4. Per-set used_mask as uint32_t with atomicOr (not bool array)
// 5. Warp-uniform control flow (no lane-local branches)
//
// Data structures:
// - cpu_to_gpu_slot_map[MAX_PAGE_ID] -> GPU slot index (-1 if not cached)
// - gpu_to_cpu_page_map[num_slots] -> CPU page ID (-1 if empty)
// - slot_stamps[num_slots] -> LRU timestamp (higher = more recent)
// - set_clock[num_sets] -> monotonic clock per set
// - set_version[num_sets] -> seqlock version (even=stable, odd=writing)
// - set_used_mask[num_sets] -> uint32_t bitmask of slots used this round
//
// =============================================================================
...
// =============================================================================
__global__ void allocate_pages_hive_lockfree_kernel(
...
__global__ void zero_hive_set_data_kernel(
    uint32_t* __restrict__ set_used_mask,
    int32_t* __restrict__ evicted_cpu_pages,
    int32_t* __restrict__ overflow_flag,
    int32_t num_sets,
...
__global__ void init_hive_structures_kernel(
    uint32_t* __restrict__ slot_stamps,
    uint32_t* __restrict__ set_clock,
    uint32_t* __restrict__ set_version,
    int32_t num_slots,
...
void allocate_pages_hive_lockfree(
    ...
    // Zero per-round data
    zero_hive_set_data_kernel<<<num_blocks, threads_per_block, 0, stream>>>(
        set_used_mask_ptr,
    ...
    allocate_pages_hive_lockfree_kernel<<<num_blocks, threads_per_block, 0, stream>>>(
...
void init_hive_structures(
    ...
    init_hive_structures_kernel<<<blocks, threads, 0, stream>>>(
        slot_stamps_ptr,
        set_clock_ptr,
        set_version_ptr,
        num_slots,
