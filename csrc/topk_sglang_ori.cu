 ...
 */
 ...
 // NOTE: TopK is a compile-time constant here because shared-memory
 // allocations inside the transform kernels depend on it. We drop it to
 // 30 to match the vortex benchmark's --topk-val 30 configuration. The
 // transform kernels (decode/prefill/prefill_ragged) still carry a manual
 // unroll that assumes TopK==2048; that code path is unreachable from the
 // bench (we only invoke fast_topk_interface), so the corresponding
 // static_asserts have been removed below.
 ...
 __global__ __launch_bounds__(kThreadsPerBlock)  // decode
   ...
   if (length <= TopK) {
     ...
     // (static_asserts removed because TopK != 2048 in this build; the
     // manual unroll below is unreachable from bench_topk.py which only
     // calls fast_topk_interface, not this transform variant.)
 ...
 // The public interface functions below collide by name with identically
 // named symbols in topk_sglang.cu. Wrap them in `sglang_ori` so both
 // translation units can be linked into the same vortex_torch_C extension.
 namespace sglang_ori {
...
// ======================================================================
// Thin vortex_torch_C adapter: accepts the same CSR-ish inputs as
// topk_output_sglang so bench_topk.py can treat the original SGLang kernel
// as an alternate baseline. The ori kernel has TopK baked in as a compile-
// time constant; this build sets it to 30 to match --topk-val 30.
// ======================================================================
void topk_output_sglang_ori(
    const at::Tensor& x,                 // [total_dense, 1, 1] or [total_dense], bf16/fp32
    const at::Tensor& dense_kv_indptr,   // int32 [eff_bs + 1]  (unused — synthetic bench rows are uniform)
    at::Tensor&       indices_out,       // int32 [eff_bs, TopK]
    ...
    const int64_t     max_num_pages)
{
    ...
    TORCH_CHECK(indices_out.is_cuda(), "indices_out must be a CUDA tensor");
    TORCH_CHECK(indices_out.scalar_type() == at::ScalarType::Int,
                "indices_out must be int32");
    TORCH_CHECK(topk_val == static_cast<int64_t>(30),
                "topk_output_sglang_ori: this build of the ori kernel hard-codes TopK=30; "
                "rebuild topk_sglang_ori.cu with a different TopK if you need another value. "
                "Got topk_val=", topk_val);
    TORCH_CHECK(indices_out.dim() == 2
                    && indices_out.size(0) == eff_batch_size
                    && indices_out.size(1) == 30,
                "indices_out must be [eff_batch_size, 30]");

    // ori kernel requires fp32 [B, stride] scores. Caller typically passes
    // the bf16 score tensor; we materialize an fp32 view once per call.
    at::Tensor score_f32;
    if (x.scalar_type() == at::ScalarType::Float) {
        score_f32 = x.contiguous().view({eff_batch_size, max_num_pages});
    } else if (x.scalar_type() == at::ScalarType::BFloat16) {
        score_f32 = x.to(at::kFloat).contiguous().view({eff_batch_size, max_num_pages});
    } else {
        TORCH_CHECK(false, "topk_output_sglang_ori: unsupported dtype ", x.scalar_type());
    }

    auto opts_i32 = at::TensorOptions().dtype(at::kInt).device(x.device());
    const int32_t usable_len =
        static_cast<int32_t>(max_num_pages - reserved_bos - reserved_eos);
    at::Tensor lengths    = at::full({eff_batch_size}, usable_len, opts_i32);
    at::Tensor row_starts = at::full({eff_batch_size},
                                     static_cast<int32_t>(reserved_bos), opts_i32);

    sglang_ori::fast_topk_interface(
        score_f32, indices_out, lengths,
