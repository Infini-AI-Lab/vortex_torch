"""block_sparse_attention + approxTopK(tolerate_ratio=0.25) — trtllm indexer.

Variant 3/10 of a tolerate_ratio sweep on AIME24. Identical to the built-in
...
op is ``approxTopK(tolerate_ratio=0.25)`` instead of exact ``topK()``.
...
@register("claude_opus_5_batch_0_id2_cls")
class ClaudeOpus5Batch0Id2Cls(vFlow):
    def __init__(self):
        super().__init__()
        # Indexer-side ops
        self.gemm = GeMM()
