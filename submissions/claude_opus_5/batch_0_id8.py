"""block_sparse_attention + approxTopK(tolerate_ratio=0.85) — trtllm indexer.

Variant 9/10 of a tolerate_ratio sweep on AIME24. Identical to the built-in
...
op is ``approxTopK(tolerate_ratio=0.85)`` instead of exact ``topK()``.
...
@register("claude_opus_5_batch_0_id8_cls")
class ClaudeOpus5Batch0Id8Cls(vFlow):
    def __init__(self):
        super().__init__()
        # Indexer-side ops
        self.gemm = GeMM()
        self.mean = Mean(dim=1)
        self.output_func = approxTopK(tolerate_ratio=0.85)
