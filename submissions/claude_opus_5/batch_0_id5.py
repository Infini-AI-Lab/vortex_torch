"""block_sparse_attention + approxTopK(tolerate_ratio=0.55) — trtllm indexer.

Variant 6/10 of a tolerate_ratio sweep on AIME24. Identical to the built-in
...
op is ``approxTopK(tolerate_ratio=0.55)`` instead of exact ``topK()``.
...
@register("claude_opus_5_batch_0_id5_cls")
class ClaudeOpus5Batch0Id5Cls(vFlow):
    def __init__(self):
        super().__init__()
        # Indexer-side ops
        self.gemm = GeMM()
        self.mean = Mean(dim=1)
        self.output_func = approxTopK(tolerate_ratio=0.55)
