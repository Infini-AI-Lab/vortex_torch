"""block_sparse_attention + approxTopK(tolerate_ratio=0.0) — trtllm indexer.

Variant 1/10 of a tolerate_ratio sweep on AIME24. Identical to the built-in
...
op is ``approxTopK(tolerate_ratio=0.0)`` instead of exact ``topK()``.

tolerate_ratio=0.0 is exact: the gate then requires all k blocks to be strict
pass-1 winners, so the refinement round is skipped only when it would change
nothing.
...
@register("claude_opus_5_batch_0_id0_cls")
class ClaudeOpus5Batch0Id0Cls(vFlow):
    def __init__(self):
        super().__init__()
        # Indexer-side ops
        self.gemm = GeMM()
        self.mean = Mean(dim=1)
        self.output_func = approxTopK(tolerate_ratio=0.0)
