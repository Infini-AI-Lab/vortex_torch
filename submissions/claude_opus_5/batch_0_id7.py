"""block_sparse_attention + approxTopK(tolerate_ratio=0.75) — trtllm indexer.

Variant 8/10 of a tolerate_ratio sweep on AIME24. Identical to the built-in
...
op is ``approxTopK(tolerate_ratio=0.75)`` instead of exact ``topK()``.
...
@register("claude_opus_5_batch_0_id7_cls")
class ClaudeOpus5Batch0Id7Cls(vFlow):
    def __init__(self):
        super().__init__()
        # Indexer-side ops
        self.gemm = GeMM()
        self.mean = Mean(dim=1)
        self.output_func = approxTopK(tolerate_ratio=0.75)
