...
| `vortex_host_kv_policy` | eviction policy for the GPU cache over the host KV tier: `"lru"` (default), `"fifo"`, `"full"`, `"none"`. `"none"` **disables caching** — the pool becomes per-step staging and every selected block is re-copied from the host each step (verified: an unchanged selection copies 256 blocks/step under `none` versus 256-then-zero under `lru`). It is the control for measuring what the cache buys, and the fallback if a caching bug is ever suspected; everything else (slot allocation, pinning, the reserved zero block) is identical, so it isolates *reuse*. The pool is **set-associative** (32 ways, block id → set by `id % n_sets`), modelled on OneFlow's `one_embedding` cache, so a miss costs one set rather than a scan of the pool — measured flat in capacity (0.16 → 0.14 ms/step from 256 to 16384 blocks, against a 32x cliff for the old global cursor). `lru` keeps a re-referenced working set (measured 96 vs 113 PCIe copies against `fifo` when hot and cold blocks contend for one set); `fifo` ignores reads, so one scan-heavy step cannot flush the pool; `full` never evicts and reports an overflow if the pool is too small, i.e. a pure prefetch buffer. An entry the cache cannot place resolves to a **reserved zero block**, so it contributes no information rather than another block's data, and is counted in the `overflow` statistic. |
