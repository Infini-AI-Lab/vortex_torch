...
class ServerArgs:
    ...
    # -------------------------------------------------------------------------
    # All hyper-parameters live in a single, vortex_torch-owned config object
    # (vortex_torch.engine.sgl.config.VortexConfig). The legacy flat
    # `enable_vortex_sparsity` / `vortex_*` reads are served by __getattr__
    # below; flat kwargs (sgl.Engine(vortex_topk_val=...)) are folded into this
    # field by vortex_torch's install_serverargs_adapter().
    vortex: A[
        Optional[Any],
        Arg(
            cli_name="vortex-config",
