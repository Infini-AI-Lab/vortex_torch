...
def enable_fused_set_kv_buffer(forward_batch: ForwardBatch):
    ...
    [VORTEX HOOK] The fused kernel writes K/V directly into
    ``token_to_kv_pool.get_{key,value}_buffer(layer_id).view(num_tokens, -1)``
    ...
    ``VortexCachePool``) MUST opt out, or the cache silently corrupts — decode
    then reads from the wrong addresses and either returns garbage (≈0%
    accuracy) or trips a CUDA illegal-memory-access. Such pools set
    ``supports_fused_set_kv_buffer = False`` on themselves. The opt-out gates
    BOTH the CUDA and the HIP branch (the HIP branch has no dtype/pool guard of
    its own, so without this it would fire for any vortex pool).
    """
    pool = get_token_to_kv_pool()
    if not getattr(pool, "supports_fused_set_kv_buffer", True):
        return False
    return (
        _is_cuda
