        ...
        for i, (decode_req, poll) in enumerate(zip(self.queue, polls)):
            ...
            if (
                ...
                if isinstance(decode_req.req.finished_reason, FINISH_ABORT):
                    ...
                    # [VORTEX HOOK] PD disaggregation (Option B): the per-page
                    # auxiliary cache (centroids/envelopes) is NOT transferred
                    # from prefill — rebuild it on the decode side from the
                    # just-received K/V before this req's first decode step.
                    # No-op for non-vortex pools.
                    _pool = self.scheduler.token_to_kv_pool_allocator.get_kvcache()
