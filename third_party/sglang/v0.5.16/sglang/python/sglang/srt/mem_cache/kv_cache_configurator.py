...
class KVCacheConfigurator:
    ...
    # [VORTEX HOOK] the owning ModelRunner. vortex's KV pool is compiled against
    # the runner (it reads block_size / sparse_attention / req_to_token_pool), so
    # the vortex branch in _build_token_to_kv_pool needs it. None when vortex
    # sparsity is off, which is every non-vortex code path.
    ...
    def resolve_max_num_reqs(self, token_capacity: int) -> int:
        ...
        # [VORTEX HOOK] hard-cap at 1024 under vortex sparsity. The per-request
        # compiled cache scratch scales with max_running_requests, so the stock
        # 2048..4096 band reserves far more than a vortex run needs. Applied
        # after the clamp above (upstream's order is max(min(..), 2048), so
        # substituting the ceiling in-place would collapse to 2048).
        if self.server_args.enable_vortex_sparsity:
            estimated = min(estimated, 1024)
