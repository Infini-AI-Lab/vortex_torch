...
def topk_sparse_attention_varlen(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    topk_idx: torch.Tensor,
    ...
    """Topk sparse attention with separate Q and K sequence lengths (for extend/prefill).

    Same as topk_sparse_attention but accepts separate cu_seqlens for Q and K.
    Useful when Q only covers new tokens while K covers all tokens (prefix + new).

    Args:
        q (torch.Tensor): shape [total_q, num_q_heads, head_dim]
        k (torch.Tensor): shape [total_k, num_kv_heads, head_dim]
        v (torch.Tensor): shape [total_k, num_kv_heads, head_dim]
        topk_idx (torch.Tensor): topk block idx for each query, shape [num_kv_heads, total_q, topk]. -1 means padding.
        block_size (int): key value block size.
        cu_seqlens_q (torch.Tensor): shape [batch_size + 1], cumulative Q sequence lengths.
        cu_seqlens_k (torch.Tensor): shape [batch_size + 1], cumulative K sequence lengths.
        softmax_scale (Optional[float], optional): Defaults to None, means 1/sqrt(head_dim).

    Returns:
        torch.Tensor: attention output, shape [total_q, num_q_heads, head_dim]
    """
