...
Supports optional `scale_page_map` for CPU VTX: when KV data lives in GPU staging
(indexed by staging slot IDs) but scales live in a persistent buffer (indexed by
CPU flat page IDs), the kernel can resolve the indirection on-the-fly.
"""
...
def _fwd_kernel_int8_stage1(
    ...
    When USE_SCALE_PAGE_MAP=True, scale lookups use:
        scale_page = Scale_Page_Map[data_page]
    instead of data_page directly. This supports CPU VTX where KV data
    is in staging slots but scales are indexed by CPU flat page IDs.
    """
    ...
    if split_kv_end > split_kv_start:
        ...
        for start_n in range(split_kv_start, split_kv_end, BLOCK_N):
            ...
            # Load page indices from kv_indices (data page IDs)
            # Cast to int64 to prevent overflow: page * PAGE_SIZE * HEAD_DIM can exceed INT32_MAX
            data_page_ids = tl.load(
                kv_indices + cur_batch_kv_start_idx + page_indices_in_seq,
                mask=mask_n,
                other=0,
            ).to(tl.int64)
            in_page_offsets_i64 = in_page_offsets.to(tl.int64)

            # Flat token location for KV data
            kv_loc = data_page_ids * PAGE_SIZE + in_page_offsets_i64

            # Resolve scale page IDs (may differ from data page IDs)
            if USE_SCALE_PAGE_MAP:
                scale_page_ids = tl.load(
                    Scale_Page_Map + data_page_ids,
                    mask=mask_n,
                    other=0,
                ).to(tl.int64)
                scale_loc = scale_page_ids * PAGE_SIZE + in_page_offsets_i64
            else:
                scale_loc = kv_loc

            # Load int8 K and dequantize
            offs_buf_k = kv_loc[:, None] * stride_buf_kbs + offs_d[None, :].to(tl.int64)
            k_int8 = tl.load(
                K_Buffer + offs_buf_k,
            ...
            offs_buf_v = kv_loc[:, None] * stride_buf_vbs + offs_dv[None, :].to(tl.int64)
            v_int8 = tl.load(
                V_Buffer + offs_buf_v,
...
def paged_decode_int8(
    ...
    If scale_page_map is provided, scale lookups use indirection:
        scale_page = scale_page_map[data_page]
    This supports CPU VTX where KV data is in staging slots but scales are indexed
    by CPU flat page IDs stored in a persistent GPU buffer.
    """
    batch = q.shape[0]
    head_num = q.shape[1]
    Lk = q.shape[2]
    ...
    # Triton requires a valid pointer even when not used
    if scale_page_map is None:
        scale_page_map = kv_indices  # dummy, won't be accessed
